function [DAS_result,PSF,hn] = DAS(N,z0,f,phi,rn,source,SNR)
...
for k = 1:size(source,1)
    ...
    Rmn = Rmn + squeeze(gn(source(k,2),source(k,1),:))*squeeze(gn(source(k,2),source(k,1),:))';
end
...
for ii = 1:length(X)
    for jj = 1:length(Y)
        e = squeeze(hn(ii,jj,:));
        DAS_result(ii,jj) = dot(e,Rmn*e)/(M^2-M);
    end
end
end
